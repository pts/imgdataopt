...
* imgdataopt is idempontent: running it again on the output, with the same
...
* imgdataopt is portable: it is written in ANSI C, and it's ported
...
* imgdataopt is self-contained: it doesn't have any external dependencies
  other than libc (the C standard library) and zlib (-lz). There is a
  also a copy of zlib bundled with imgdataopt in zlib_src. Other tools such
  as ImageMagick have dozens of dependencies, with libpng (-lpng) as a
  minimum. sam2p also has the dependency on png22pnm (or pngtopnm).
...
* imgdataopt can read PNG files generated by pdfsizeopt.
...
* imgdataopt can write PNG-like files without a per-row predictor specified.
  ...
  To get this non-conforming PNG output, use `imgdataopt -j:ext
  -c:zip:none'.
